...
> **The fixture is synthetic.** `fixtures/tanker_i5.jsonl` was hand-authored to
> exercise the UI without a running pipeline. Its camera ID, location, chemical
> release, dispatched units and telemetry are all invented — no Washington State
> DOT feed was used and no such incident occurred. The IDs are realistic on
> purpose, so that the dashboard looks like the real thing; do not mistake the
> replay for a recording.
