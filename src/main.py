...
def run_live_stream_simulation(video_path: str, location: Optional[str], speed: float):
    ...
    for event in orchestrator.stream_incident(video_path, location_hint=location, speed_multiplier=speed):
        ...
        if status == "ANOMALY_DETECTED":
            ...
            print(f"\n{prefix} {desc}")
            print("\033[1;36m⚡ Dispatch rules: triggering hazmat, traffic and comms sub-agents...\033[0m")
            ...
            print(f"\n{prefix} {desc}")
            print("\033[1;33m⚡ Dispatch rules: roadway safety advisory triggered.\033[0m")
            record = event.get("incident_record")
            if record:
                print_rich_incident_report(record)
