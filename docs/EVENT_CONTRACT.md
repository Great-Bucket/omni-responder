...
The orchestrator's decision step. Despite the name, this is **not** model
output: the decisions come from deterministic rules in `dispatch-rules`
(`dashboard/adapter.py`), and `text` is a formatted explanation of a branch that
was already taken. Keep `text` under ~200 chars; it renders in a log line.
